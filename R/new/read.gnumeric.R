...
read.gnumeric.sheet <-
  function(file,       ## 'filename.gnumeric'
           ...
           field.format='raw';        ## For: ssconvert -O "format=value".
...
#               " fd://1 ", sep='');
  ...
  cmd <- paste(ssconvert.full.path,
               ...
               ' -O "locale=',locale,' format=',field.format,' separator=, eol=unix sheet=', sheet.name.with.quotes ,'"',
               range , 
               IMPORT.ENCODING,
               ' "', file, '"',
               " fd://1 ", sep='');
...
### gnumeric: format=raw emits 25569 for 1970-01-01
##gnumeric.date.epoch= 25569;
gnumeric.as.date <- function(x){
  v=as.numeric(as.character(x));
  v=v-gnumeric.date.epoch;
  class(v)<-'Date';
  v
}

gnumeric.as.datetime <- function(x){
  as.POSIXct(as.numeric(as.character(x))*(60*60*24)-1,  origin="1899-12-30")
}
